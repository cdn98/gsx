...
document.addEventListener('DOMContentLoaded', () => {
	...
	customElements.define(
		...
		class JoeMp3 extends HTMLElement {
			...
			render() {
				if (!this.options.url) return (this.innerHTML = '音頻地址未填寫！');
		...
		class JoeAnote extends HTMLElement {
			constructor() {
				...
				this.options = {
					...
					content: this.getAttribute('content') || '標簽按鈕'
				};
				this.innerHTML = `
		...
		class JoeCloud extends HTMLElement {
			constructor() {
				...
				this.options = {
					...
					title: this.getAttribute('title') || '默認標題',
					url: this.getAttribute('url'),
				...
				this.innerHTML = `
					<span class="joe_cloud">
						...
						<div class="joe_cloud__describe">
							...
							<div class="joe_cloud__describe-type">來源：${type[this.options.type] || '默認網盤'}${this.options.password ? ' | 提取碼：' + this.options.password : ''}</div>
						</div>
	...
	customElements.define(
		...
		class JoeHide extends HTMLElement {
			...
			render() {
				this.innerHTML = '<span class="joe_hide">此處內容作者設置了 <i class="joe_hide__button">回復</i> 可見</span>';
		...
		class JoeCardDefault extends HTMLElement {
			constructor() {
				...
				this.options = {
					...
					content: _temp.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, '') || '卡片內容'
				};
				const htmlStr = `
		...
		class JoeMessage extends HTMLElement {
			constructor() {
				...
				this.options = {
					...
					content: this.getAttribute('content') || '消息內容'
				};
				this.innerHTML = `
		...
		class JoeCallout extends HTMLElement {
			constructor() {
				...
				this.options = {
					...
					content: _temp.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, '') || '標註內容'
				};
				const htmlStr = `
		...
		class JoeCollapse extends HTMLElement {
			constructor() {
				...
				this.querySelectorAll('.joe_collapse__item').forEach(item => {
					const label = item.getAttribute('label') || '折疊標題';
	...
	customElements.define(
		...
		class JoeCopy extends HTMLElement {
			constructor() {
				super();
				this.options = {
					showText: this.getAttribute('showText') || '點擊復製',
					copyText: this.getAttribute('copyText') || '默認文本'
				};
				...
				if (typeof ClipboardJS !== 'undefined' && typeof Qmsg !== 'undefined') {
					new ClipboardJS(button, { text: () => this.options.copyText }).on('success', () => Qmsg.success('復製成功！'));
				} else {
					button.addEventListener('click', () => alert('該功能請前往前臺查看！'));
				}
			}
		}
	);
