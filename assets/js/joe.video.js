    ...
    function initVideoList() {
        ...
        $.ajax({
            ...
            success(res) {
                ...
                if (!res.data.class.length) return $('.joe_video__type-list').html(`<li class="error">暫無數據！</li>`);
        ...
        function renderDom() {
            ...
            $.ajax({
                ...
                success(res) {
                    if (res.code !== 1) return $('.joe_video__list-item').css('display', 'block').html('<p class="error">數據加載失敗！請檢查！</p>');
                    if (!res.data.list.length) {
                        $('.joe_video__list-item').css('display', 'block').html('<p class="error">暫無數據！</p>');
                    } else {
                        let htmlStr = '';
        ...
        function initPagination() {
            ...
            if (queryData.pg < pagecount) htmlStr += `<li class="joe_video__pagination-item" data-pg="${pagecount}">末頁</li>`;
    ...
    function initVideoDetail() {
        ...
        $.ajax({
            ...
            success(res) {
                ...
                if (!res.data.list.length) return $('.joe_video__detail-info').html(`<p class="error">數據加載失敗！請檢查！</p>`);
                ...
                $('.joe_video__detail-info').html(`
					...
					<dl class="description">
						...
						<dd><span class="muted">類型：</span><span class="text">${item.vod_class || '未知'}</span></dd>
						<dd><span class="muted">主演：</span><span class="text">${item.vod_actor || '未知'}</span></dd>
						<dd><span class="muted">導演：</span><span class="text">${item.vod_director || '未知'}</span></dd>
						<dd><span class="muted">簡介：</span><span class="text">${getContent(item)}</span></dd>
					</dl>
				`);
                /* 设置视频播放标题 */
                ...
                for (let [key, value] of maps) {
                    ...
                    htmlStr += `
						<div class="joe_video__source joe_video__contain">
							<div class="joe_video__contain-title">播放線路 ${index}</div>
