document.addEventListener('DOMContentLoaded', () => {
    ...
    $.ajax({
        ...
        success(res) {
            if (res.code !== 1) return $('.joe_wallpaper__type-list').html('<li class="error">壁紙抓取失敗！請聯系作者！</li>');
            if (!res.data.length) return $('.joe_wallpaper__type-list').html(`<li class="error">暫無數據！</li>`);
            let htmlStr = '';
                ...
                res.data.forEach(_ => {
                    htmlStr += `
                        <a class="item animated bounceIn" data-fancybox="gallery" href="${_.url}">
                            <img width="100%" height="100%" class="lazyload" src="${Joe.LAZY_LOAD}" data-src="${_.img_1024_768 || _.url}" alt="壁紙">
                        </a>`;
                });
    ...
    function initPagination() {
        ...
        if (queryData.start < total - queryData.count) htmlStr += `<li class="joe_wallpaper__pagination-item" data-start="${total - queryData.count}">末頁</li>`;
